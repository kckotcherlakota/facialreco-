    ...
    for face_encoding in face_encodings:
        ...
        if True in matches:
            ...
            if int(name) in range(1,61):
                sheet.cell(row=int(name)+3, column=1).value = "Present"
                sheet.cell(row=1, column=1).value = "attendance for "+str(today)+"-"+str(month)+"-19"
            
            while x<=61:
                if sheet.cell(row=x+2, column=1).value !="Present":
                    sheet.cell(row=x+2, column=1).value ="Absent"
                x=x+1
